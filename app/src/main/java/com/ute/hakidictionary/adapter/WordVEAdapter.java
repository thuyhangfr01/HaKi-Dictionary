...
import com.ute.hakidictionary.activities.WordDetailVEActivity;
...
public class WordVEAdapter extends RecyclerView.Adapter<WordVEAdapter.WordViewHolder> {
    private List<WordSearch> mlistWord;
    private Context context;
    String createdTime;
    int idUser;
    public WordVEAdapter(Context context, List<WordSearch> mlistWord) {
    ...
    @Override
    public void onBindViewHolder(@NonNull WordVEAdapter.WordViewHolder holder, int position) {
        ...
        holder.layoutSearch.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Intent intent = new Intent(context, WordDetailVEActivity.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                intent.putExtra("wordName", wordSearch.getName());
                intent.putExtra("wordId", wordSearch.getId());
                context.startActivity(intent);
                addListHistory(idUser, wordSearch.getId(), createdTime);
            }
        });
    }
    ...
    public void addListHistory(int idUser, int idDicEV, String createdTime){
        ...
        try {
            ...
            jsonObject.put("idDicVE", idDicEV);
            jsonObject.put("dateSearch", createdTime);
        } catch (JSONException e) {
            e.printStackTrace();
        }

        AndroidNetworking.post("http://ec2-3-144-36-186.us-east-2.compute.amazonaws.com:3030/api/historyVieEng/addHistoryVieEng")
